

const Myskills = () => {
  return (
    <div className=" bg-[#161616] h-[560px] w-[100%] ">
        <div className="flex gap-10 ">
     <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div >
        <img src='https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/C_Programming_Language.svg/926px-C_Programming_Language.svg.png'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">C Programming</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div>
        <img src='https://cdn-icons-png.flaticon.com/512/919/919827.png'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">HTML</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div>
        <img src='https://cdn-icons-png.flaticon.com/512/732/732190.png'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">CSS</h1>
         
       </div>
      </div>
      </div>
      <div className="flex gap-10">
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div >
        <img src='https://cdn-icons-png.flaticon.com/512/5968/5968292.png'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">Java Script</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div>
        <img src='https://cdn.iconscout.com/icon/free/png-256/free-react-1-282599.png?f=webp'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">React JS</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4 " >
          
          <div>
        <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8rK-LE52N0d05lb7yCImZyEfwaTKb_KLj2A0IfKolsA&s'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">Tailwind CSS</h1>
         
       </div>
      </div>
      </div>
      <div className="flex gap-10">
     <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div>
        <img src='https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">Github</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div>
        <img src='https://cdn-icons-png.flaticon.com/512/5968/5968381.png'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">Type Script</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl h-[60px] w-[25%] ml-[50px] mt-[50px] flex gap-4" >
          
          <div>
        <img src='data:image/jpeg;base64,'
          className="h-[50px] bg-white w-[40px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 text-[25px] font-bold">Bootstrap</h1>
         
       </div>
      </div>
      </div>
      <div className="flex justify-center mr-[50px] mt-[90px]">
     <button className="flex justify-center text-white  text-[20px] hover:bg-white hover:text-black border-2 w-[120px] h-[40px] font-semibold">My Projects</button>
     </div>
     </div>
   
  )
}

export default Myskills